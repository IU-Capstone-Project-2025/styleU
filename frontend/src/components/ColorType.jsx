...
export default function ColorType() {
  ...
  const analyzeImage = async () => {
    ...
    try {
      ...
      let translatedType = result.color_type?.toLowerCase() || '';
      if (i18n.language === 'ru' && translatedType in colorTranslation) {
        translatedType = colorTranslation[translatedType];
      }

      const parsedResult = {
        type: translatedType.toUpperCase() || t('colorType.notDefined'),
        meaning: rec.about || '',
        suitableColors: {
          ...
          <div className="w-full md:w-[40%] flex flex-col items-center md:items-start">
            <div className="w-[90%] max-w-[320px]">
              <label htmlFor="file-upload" className="cursor-pointer w-full block">
                {!image ? (
                  <div className="border border-black rounded-lg w-full aspect-square flex flex-col items-center justify-center bg-cover bg-center bg-no-repeat"
                ...
                <div className="border border-gray-300 rounded-xl px-4 md:px-6 py-4 md:py-5 bg-white shadow-md w-full max-w-sm text-center transition-all">
                  ...
                  <div className="flex justify-center gap-5">
                    <button
                      ...
                      className={`w-10 h-10 md:w-11 md:h-11 rounded-full border flex items-center justify-center transition duration-300 ${feedback === 'like' ? 'border-gray-400 ring-2 ring-gray-300 bg-white shadow-md' : 'hover:bg-gray-100 border-gray-300'}`}>
                      <img src={like} alt="Like" className="w-5 h-5" />
                    </button>
                    <button
                      onClick={() => sendFeedback('dislike')}
                      className={`w-10 h-10 md:w-11 md:h-11 rounded-full border flex items-center justify-center transition duration-300 ${feedback === 'dislike' ? 'border-gray-400 ring-2 ring-gray-300 bg-white shadow-md' : 'hover:bg-gray-100 border-gray-300'}`}>
            ...
            <div className="w-full max-w-full">
              ...
              {colorType ? (
                ['meaning', 'suitableColors', 'unsuitableColors'].map((section) => {
                  ...
                  const title = t(`colorType.${section === 'meaning' ? 'meaning' : section === 'suitableColors' ? 'suitable' : 'unsuitable'}`);
                  const content =
                    section === 'meaning' ? (
